...
class CollectingImage:
    def __init__(
        self,
        save_path="dataset/clean_images",
        count=10,
        width=3060,
        height=4080,
        max_workers=5,
        ...
        if self.source == "pexels" and not self.pexels_api_key:
            raise ValueError(
                "Pexels source selected, but PEXELS_API_KEY environment variable is not set."
            )
    ...
    def _get_start_index(self):
        ...
        for f in existing_files:
            try:
                ...
                self.logger.debug(f"Could not parse index from filename: {f.name}")
    ...
    def _fetch_pexels_urls(self) -> list:
        if not self.pexels_api_key:
            return []
        self.logger.info(
            f"Fetching {self.count} image URLs from Pexels for query: '{self.pexels_query}'..."
        )
        urls = []
        page = 1
        per_page = 80
        ...
        while len(urls) < self.count:
            try:
                ...
                data = response.json()
                page_urls = [
                    photo["src"]["original"] for photo in data.get("photos", [])
                ]
                if not page_urls:
                    self.logger.warning(
                        "No more photos found on Pexels for this query."
                    )
                    break
    ...
    def download_images(self):
        self.logger.info(
            f"Starting download of {self.count} images from '{self.source}'."
        )
        if self.source == "pexels":
            image_urls = self._fetch_pexels_urls()
            if not image_urls:
                self.logger.error(
                    "Could not fetch any image URLs from Pexels. Aborting."
                )
                return
            tasks_to_run = [
                (url, self.start_index + i + 1) for i, url in enumerate(image_urls)
            ]
        elif self.source == "picsum":
            base_url = "https://picsum.photos"
            tasks_to_run = [
                (
                    f"{base_url}/{self.width}/{self.height}?random={self.start_index + i + 1}",
            ...
            self.logger.error(
                f"Unknown source: '{self.source}'. Please use 'pexels' or 'picsum'."
            )
            return
        successful_downloads = 0
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._download_and_process_url, url, index): index
                for url, index in tasks_to_run
            }
            pbar = tqdm(
        ...
        self.logger.info(
            f"Download complete: {successful_downloads}/{self.count} images were successful."
        )
