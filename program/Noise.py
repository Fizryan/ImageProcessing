...
def _add_salt_and_pepper_noise(image_array, noise_level):
    ...
    num_pixels_to_affect = int(
        noise_level * image_array.shape[0] * image_array.shape[1]
    )

    salt_coords_y = np.random.randint(
        0, image_array.shape[0], num_pixels_to_affect // 2
    )
    salt_coords_x = np.random.randint(
        0, image_array.shape[1], num_pixels_to_affect // 2
    )
    noisy_array[salt_coords_y, salt_coords_x] = [255, 255, 255]

    pepper_coords_y = np.random.randint(
        0, image_array.shape[0], num_pixels_to_affect // 2
    )
    pepper_coords_x = np.random.randint(
        0, image_array.shape[1], num_pixels_to_affect // 2
    )
...
def process_image_worker(
    ...
    # Set seeds for reproducibility for this specific image
    np.random.seed(seed)
    random.seed(seed)
        ...
        try:
            ...
            with Image.open(image_path) as img, Image.open(mask_path) as mask:
                ...
                if noise_type == "gaussian":
                    noisy_array_full = _add_gaussian_noise(original_array, noise_level)
                elif noise_type == "salt_pepper":
                    noisy_array_full = _add_salt_and_pepper_noise(
                        original_array, noise_level
                    )
                else:
                ...
                final_array = np.where(
                    np.expand_dims(apply_noise_mask, axis=-1),
                    noisy_array_full,
                    original_array,
                )
            ...
            return (
                "ERROR",
                f"Failed to process {image_path.name} after {max_retries} attempts: {e}",
            )
...
class NoiseGenerator:
    def __init__(
        ...
        noise_dir="dataset/noisy_images",
        noise_level=0.1,
        noise_type="random",
        max_workers=None,
        max_retries=3,
    ...
    def _verify_image_pairs(self, image_files: List[Path]) -> List[Tuple[Path, Path]]:
        ...
        for path in pbar:
            ...
            if not mask_path.exists():
                self.logger.warning(
                    f"Skipping {path.name}: No corresponding mask file found in {self.mask_dir}"
                )
                continue
    ...
    def generate_noisy_images(self):
        ...
        if not valid_image_pairs:
            self.logger.warning(
                "No valid image-mask pairs left to process after verification."
            )
            return

        self.logger.info(
            f"Starting to process {len(valid_image_pairs)} valid image-mask pairs."
        )
