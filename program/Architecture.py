...
class DownsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Conv2d(
            in_channels * 4, out_channels, kernel_size=3, padding=1, bias=True
        )
...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels, out_channels * 4, kernel_size=3, padding=1, bias=True
        )
...
class SOTARestorationUNet(nn.Module):
    def __init__(
        ...
        self.intro = nn.Conv2d(
            in_channels, base_channels, kernel_size=3, padding=1, bias=True
        )
        ...
        self.dec3 = nn.Sequential(
            nn.Conv2d(base_channels * 8, base_channels * 4, kernel_size=1, bias=True),
        ...
        self.dec2 = nn.Sequential(
            nn.Conv2d(base_channels * 4, base_channels * 2, kernel_size=1, bias=True),
        ...
        self.dec1 = nn.Sequential(
            nn.Conv2d(base_channels * 2, base_channels, kernel_size=1, bias=True),
            ResBlockNoNorm(base_channels),
            ResBlockNoNorm(base_channels),
        )

        self.final_conv = nn.Conv2d(
            base_channels, out_channels, kernel_size=3, padding=1, bias=True
        )
    ...
    def forward(self, x, return_internals=False):
        ...
        if return_internals:
            return output, {"bottleneck": b}

        return output


class PatchGANDiscriminator(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, num_layers=3):
        super().__init__()
        layers = [
        ...
        layers.extend(
            [
                nn.Conv2d(
                    in_ch, out_ch, kernel_size=4, stride=1, padding=1, bias=False
                ),
                nn.InstanceNorm2d(out_ch),
                nn.LeakyReLU(0.2, inplace=True),
            ]
